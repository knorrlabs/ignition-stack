"""Hub-and-spoke architecture: 1 central hub + N spoke gateways.

This mirrors Ignition's documented Hub and Spoke architecture. Edge gateways
typically run as the spokes (lightweight, deployed close to the data); the hub
aggregates them. Per the design's tiered-advisory decision, the spoke count
drives an advisory: green ≤4, yellow 5-8, red ≥9 (refuses without ``--force``).
The advisory itself lives in :mod:`ignition_stack.architectures.advisory`; this
architecture wires it into the build path so a bare ``build()`` raises
``ArchitectureError`` on the red tier unless ``options.force`` is set.

Reverse-proxy / database / services flow through the standard ``ArchOptions``
contract; a hub-and-spoke stack is just a multi-gateway arrangement with a
specific role layout and the advisory gate.
"""
...
class ArchitectureError(Exception):
    """Raised when an architecture's preconditions are not met.
...
class HubAndSpokeArchitecture:
    ...
    def build(self, name: str, options: ArchOptions) -> ProjectConfig:
        ...
        if advisory.tier == "red" and not options.force:
            raise ArchitectureError(advisory.message, advisory=advisory)
...
architecture: Architecture = register(HubAndSpokeArchitecture())
