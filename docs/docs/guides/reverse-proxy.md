...
description: Route gateways through a Traefik reverse proxy instead of host ports, joining a proxy you already run or scaffolding inductive-automation/traefik-reverse-proxy.
---
...
| scaffold | `--reverse-proxy scaffold` | Also lays down the [inductive-automation/traefik-reverse-proxy](https://github.com/inductive-automation/traefik-reverse-proxy) README under `--proxy-path` (default `reverse-proxy/`) so you can stand the proxy up. |
...
> Name an existing network
  Scaffold inductive-automation/traefik-reverse-proxy
```
